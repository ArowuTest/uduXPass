...
const LoginPage: React.FC = () => {
  ...
  const handleMomoLogin = async (e: React.FormEvent<HTMLFormElement>): Promise<void> => {
    ...
    try {
      const response = await fetch(`${import.meta.env.VITE_API_BASE_URL || ''}/auth/momo/request-otp`, {
        method: 'POST',
        headers: {
  ...
  const handleOtpVerification = async (e: React.FormEvent<HTMLFormElement>): Promise<void> => {
    ...
    try {
      const response = await fetch(`${import.meta.env.VITE_API_BASE_URL || ''}/auth/momo/verify-otp`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json'
        },
